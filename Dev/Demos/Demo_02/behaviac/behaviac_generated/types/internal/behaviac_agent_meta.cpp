...
namespace behaviac
{
	...
	{
		IInstanceMember* _msg;

	public: 
		CMethod_FirstAgent_m1() : _msg(0) 
		{
		}

		CMethod_FirstAgent_m1(CMethod_FirstAgent_m1 &rhs) : CAgentMethodVoidBase(rhs) , _msg(0) 
		{
		}

		~CMethod_FirstAgent_m1()
		{
			BEHAVIAC_DELETE _msg;
		}
		...
		virtual void load(const char* instance, behaviac::vector<behaviac::string>& paramStrs)
		{
			...
			behaviac::StringUtils::StringCopySafe(kInstanceNameMax, _instance, instance);
			_msg = AgentMeta::TParseProperty<behaviac::string >(paramStrs[0].c_str());
		}

		virtual void run(Agent* self)
		{
			BEHAVIAC_ASSERT(_msg != NULL);

			const behaviac::string& pValue_msg = *(const behaviac::string*)_msg->GetValue(self, behaviac::Meta::IsVector<const behaviac::string >::Result, behaviac::GetClassTypeNumberId<const behaviac::string >());
			self = Agent::GetParentAgent(self, _instance);

			((FirstAgent*)self)->_Execute_Method_<METHOD_TYPE_FirstAgent_m1, void, const behaviac::string& >(pValue_msg);
		}
	};
	...
	public:
		...
		virtual bool load()
		{
			AgentMeta::SetTotalSignature(3743666655u);
			...
			// FirstAgent
			meta = BEHAVIAC_NEW AgentMeta(2316541573u);
